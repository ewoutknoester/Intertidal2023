...
NOAA <- read.table(file = "https://coralreefwatch.noaa.gov/product/vs/data/kenya.txt", header = TRUE, skip = 21, sep = "")
...
NOAA.2 <- NOAA.1[NOAA.1$Date >= "2023-02-14" & NOAA.1$Date <= "2023-06-21", ]

# HOBO
# Load data HOBO logger
HOBO <- read_excel("~/Documents/WUR/MASTER/#SARAINKENYA/2.EXPERIMENT/4.Temperature/HOBO_Temp.xlsx", sheet = 1)
...
Temp.plot <- Temp[Temp$Date >= "2023-02-14" & Temp$Date <= "2023-06-21", ]
...
Temp_graph <- xyplot(data = Temp.plot, SST.max + INT.avg + SUB.avg + MMM ~ Date, 
                     ...
                     text = list(c("Intertidal (avg)", "Subtidal (avg)", "NOAA (avg)", "MMM + 1", "DHW")),
