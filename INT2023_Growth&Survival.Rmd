...
# title: "INT2023_Growth&Survival"
# authors: "Ewout Knoester, Gian Lu Dado, Sara Sottoriva"
# date: "June 27, 2023"
...
setwd("/Users/saras//Documents/WUR/MASTER/#SARAINKENYA/2.EXPERIMENT/1.Growth measurements")
getwd()
...
# Import excel sheet
raw_data <- read_excel("~/Documents/WUR/MASTER/#SARAINKENYA/2.EXPERIMENT/1.Growth measurements/INT2023_Growth&Survival_RAW.xlsx", sheet = "DATA")

# Insert new columns to calculate average SGR over a whole period while maintaining same wide-format style data after the last column of raw_data
raw.data.1 <- as.data.frame(append(raw_data, list(Date_5 = "2023-07-20", Length_5 = NA,
                                                    Width1_5 = NA, Width2_5 = NA, EV_5 = NA, Condition_5 = NA, Cause_5 = NA, SGR_5 = NA, Comments_5 = NA), after = (ncol(raw_data))))

## Calculate SGR (Specific Growth Rate) over whole period
raw.data.1$SGR_5 <- log(raw.data.1$EV_4/raw.data.1$EV_0)/as.numeric(as.Date(as.character(raw.data.1$Date_4), format="%Y-%m-%d") - as.Date(as.character(raw.data.1$Date_0), format="%Y-%m-%d"))
...
raw.data.1$Condition_5 <- raw.data.1$Condition_4
raw.data.1$Cause_5 <- raw.data.1$Cause_4

# Convert to long data frame
raw.data.2 <- as.data.frame(long_panel(raw.data.1, prefix = "_" , begin = 0, end = 5, label_location = "end"))
...
date.order = c("02/2023", "03/2023", "04/2023", "05/2023", "06/2023", "07/2023") 

raw.data.2 %<>%
  mutate(Date = Date %<>% 
           factor(levels = date.order)%>% 
  fct_recode('Average' = '07/2023'))
...
write_xlsx(clean.data, "~/Documents/WUR/MASTER/#SARAINKENYA/2.EXPERIMENT/1.Growth measurements/Analysis/Outcome/SGR/INT_Cleaning experiment.xlsx")
...
# Import excel sheet
clean.data.1 <- read_excel("~/Documents/WUR/MASTER/#SARAINKENYA/2.EXPERIMENT/1.Growth measurements/Analysis/Outcome/SGR/INT_Cleaning experiment.xlsx", sheet = 1)
...
clean.data.1$Date <- ordered(clean.data.1$Date, c("02/2023", "03/2023","04/2023","05/2023", "06/2023", "Average"))
...
# Remove IDs with missing data (comment "missing data")
#clean.data.1 <- raw.data.2[-which(raw.data.2$Comments == "missing data")]

write_xlsx(clean.data.1, "~/Documents/WUR/MASTER/#SARAINKENYA/2.EXPERIMENT/1.Growth measurements/Analysis/Outcome/SGR/INT_Cleaning experiment_new.xlsx")
```


# SGR ANALYSIS

```{r SGR: data preparation}

#Import excel sheet
clean.data.1 <- read_excel("~/Documents/WUR/MASTER/#SARAINKENYA/2.EXPERIMENT/1.Growth measurements/Analysis/Outcome/SGR/INT_Cleaning experiment_new.xlsx", sheet = 1)
...
sgr.data.1$Treatment <- factor(sgr.data.1$Treatment, c("II", "IS", "SS", "SI"))
...
sgr.data.1$Date <- ordered(sgr.data.1$Date, c("03/2023", "04/2023", "05/2023", "06/2023"))

## Exclude columns not used
sgr.data.1 <- sgr.data.1 %>%
  dplyr::select(-c("Length", "Cause", "Condition", "EV"))

## Visualize time effect - WITH March SGR
sgr.data.summary.x <- data_summary(sgr.data.1, varname = "SGR", groupnames = c("Species", "Treatment", "Date"))

ggplot(sgr.data.summary.x, aes(x = Date, fill = Treatment, y = SGR))+
...
# Remove data from February and March (we don't want to include March SGR)
sgr.data.1 <- subset(sgr.data.1, Date != "02/2023")
sgr.data.1 <- subset(sgr.data.1, Date != "03/2023")

## Visualize time effect - WITHOUT March SGR
sgr.data.summary <- data_summary(sgr.data.1, varname = "SGR", groupnames = c("Species", "Treatment", "Date"))

ggplot(sgr.data.summary, aes(x = Date, fill = Treatment, y = SGR))+
  geom_bar(stat = "identity", position = position_dodge(), na.rm = TRUE)+
  labs(y = expression(paste("SGR (", d^-1,")")))+
  scale_y_continuous(expand=c(0, 0), limits=c(-0.004, 0.024))+
  facet_grid(Species ~ Treatment)+
...
# MODELLING
### Full linear model without random factor
lm.sgr  <- gls(SGR ~ Treatment*Species*Date, method = "REML", na.action = na.omit,
                   data = sgr.data.1) # AIC = -10750.30	

### Linear mixed-effect model with random factor
# Random factor to account for non-independence of multiple coral fragments in same nursery Structure
lme.sgr.1r  <- lme(SGR ~ Treatment*Species*Date, method = "REML", na.action = na.omit, random = ~1 | Tree, data = sgr.data.1) # AIC = -10748.47

## Allowing for heterogeneity among Species (showed best residuals of all Species * Treatment combis)
lme.sgr.2r.S  <- lme(SGR ~ Treatment*Species*Date, method = "REML", na.action = na.omit, random = ~1 |
                       Position, data = sgr.data.1, weights = varIdent(form = ~1 | Species)) # AIC = -10747.17
AIC(lm.sgr, lme.sgr.1r, lme.sgr.2r.S) #The lower the value, the better the model fits; even tho the lower value is with the linear model, we choose to use the linear mixed-effect model (1 random factor) because we can't consider the fragments from the same tree as independent

## Model output
Anova(lme.sgr.1r) # all the one-way interactions (Treatment, Species, Date) are highly significant; Two-way interactions Treatment:Date and Species:Date are as well highly significant
...
mod <- lme.sgr.1r # set model of Treatment, Species and Date to be validated
...
# Post hoc comparison
hsd.TSD <- emmeans(lme.sgr.1r, specs = pairwise ~ Treatment | Date * Species, adjust = "tuckey") 
...
ggsave("INT_SGR_Treatment x Species x Date.jpeg")
...
# Post hoc comparison
hsd.T <- emmeans(lme.sgr.1r, specs = pairwise ~ Treatment, adjust = "tuckey") 
...
SGR_Treatment
ggsave("INT_SGR_Treatment.jpeg")


## PLOT 3: Species
# Get averages
sgr.summary.S <- data_summary(sgr.data.1, varname = "SGR", groupnames = c("Species"))
...
# Post hoc comparison
hsd.S <- emmeans(lme.sgr.1r, specs = pairwise ~ Species, adjust = "tuckey") 
...
SGR_Species
ggsave("INT_SGR_Species.jpeg")


## PLOT 4: Date
# Get averages
sgr.summary.D <- data_summary(sgr.data.1, varname = "SGR", groupnames = c("Date"))
...
# Post hoc comparison
hsd.D <- emmeans(lme.sgr.1r, specs = pairwise ~ Date, adjust = "tuckey") 
...
SGR_Date
ggsave("INT_SGR_Date.jpeg")


## PLOT 4: Date*Treatment 
# Get averages
sgr.summary.TD <- data_summary(sgr.data.1, varname = "SGR", groupnames = c("Date", "Treatment"))
...
hsd.sgr.TD <- emmeans(lme.sgr.1r, specs = pairwise ~ Treatment|Date, adjust = "tukey")
...
SGR_DatexTreatment
ggsave("INT_SGR_Date x Treatment.pdf") #save image


## PLOT 5: Date*Species
# Get averages
sgr.summary.SD <- data_summary(sgr.data.1, varname = "SGR", groupnames = c("Date", "Species"))
...
hsd.sgr.SD <- emmeans(lme.sgr.1r, specs = pairwise ~ Species|Date, adjust = "tukey")
#install.packages("multcompView")
...
SGR_DatexSpecies <- ggplot(sgr.summary.SD, aes(x = Date, fill = Species, y = SGR))+
  ...
  theme(
    ...
    axis.text.y=element_text(size=12, face = "bold", vjust=0.5),
    axis.ticks.length.x = unit(c(0,2), "mm"))+
  scale_x_discrete(labels= c("04/2023", "05/2023", "06/2023"))
...
# Get averages
sgr.summary.ST <- data_summary(sgr.data.1, varname = "SGR", groupnames = c("Species", "Treatment"))
...
# Perform post hoc and get significance letters (Compact Letter Display) between Treatments within each Species
hsd.sgr.ST <- emmeans(lme.sgr.1r, specs = pairwise ~ Treatment|Species, adjust = "tukey")
#install.packages("multcompView")
...
SGR_SpeciesxTreatment
ggsave("INT_SGR_Species x Treatment.jpeg") #save image


## PLOT 7: Treatment*Species 
# Get averages
sgr.summary.ST <- data_summary(sgr.data.1, varname = "SGR", groupnames = c("Treatment", "Species"))

# Create unique Finder for each Treatment*Species combination
sgr.summary.ST <- as.data.frame(append(sgr.summary.ST,
                                       list(Finder = paste(sgr.summary.ST$Treatment, sgr.summary.ST$Species, sep=":")), after = 0))
...
# Perform post hoc and get significance letters (Compact Letter Display) between Treatments within each Species
hsd.sgr.ST <- emmeans(lme.sgr.1r, specs = pairwise ~ Species|Treatment, adjust = "tukey")
#install.packages("multcompView")
letters.sgr.ST <- multcomp::cld(hsd.sgr.ST$emmeans, decreasing = TRUE, alpha = 0.05, Letters = letters) # get CLD

# Create ID and order dataframe by name (equals Finder of sgr.summary.SS dataframe)
letters.sgr.ST <- as.data.frame(append(letters.sgr.ST,
                                   list(Finder = paste(letters.sgr.ST$Treatment, letters.sgr.ST$Species, sep=":")), after = 0))
letters.sgr.ST <- letters.sgr.ST[order(letters.sgr.ST$Finder),]
letters.sgr.ST <- letters.sgr.ST %>% mutate(.group = str_squish(.group)) # remove white spaces
...
SGR_TreatmentxSpecies <- ggplot(sgr.summary.ST, aes(x = Treatment, fill = Species, y = SGR))+
  geom_bar(stat = "identity", position = position_dodge())+ 
  labs(y = expression(paste("SGR (", d^-1,")")), x = "Treatment", c("II", "SI", "SS", "IS"))+
  scale_y_continuous(limits=c(0, 0.018), breaks = c(0, 0.005, 0.01, 0.015), expand = c(0, 0))+
  geom_errorbar(aes(ymin=SGR-(1*se), ymax=SGR+(1*se)), width=.2, position=position_dodge(.9))+
  geom_text(data = sgr.summary.ST, aes(x=Treatment, y = SGR + se+0.0004, label = ""),
            vjust=0, position = position_dodge(.9))+
  scale_fill_manual(values=c("#A9A9A9", "#3A3A3A"))+
  theme_economist()+scale_colour_economist()+
  theme(
...
SGR_TreatmentxSpecies
ggsave("INT_SGR_Treatment x Species.jpeg") #save image

```

```{r SGR - HALF PLOT}
# throught the experiment we assign half of the plot to Judy and half to me, to have more consistent pictures and ImageJ analysis
# we want to check whether this division created different patterns in SGR

## SARA HALF
# creating data set with only Saras half plots
half.S <- c("SI_40", "II_36", "II_34", "SI_32", "II_41", "II_35", "SI_45", "SI_47", "SI_38", "II_39", "II_47", "SI_48", "SI_46", "SI_42", "SI_31", "II_38", "II_40", "II_48", "SI_41", "II_45", "IS_34", "IS_33", "SS_34", "IS_32", "IS_49", "IS_36", "IS_40", "SS_32", "SS_42", "SS_31", "SS_38", "IS_37", "IS_39", "SS_35", "IS_50", "IS_44", "SS_49", "IS_47", "SS_46", "IS_46")
sgr.data.S <- sgr.data %>% filter(Structure %in% half.S) 

# cleaning data
sgr.data.1.S <- subset(sgr.data.S, Condition >= 80 | is.na(Condition))
sgr.data.1.S <- subset(sgr.data.1.S, Date != "Average") # Exclude Average-data
sgr.data.1.S <- subset(sgr.data.1.S, !(is.na(SGR))) # Exclude SGR with NA
sgr.data.1.S <- sgr.data.1.S %>% separate(Structure, c('Treatment', 'Tree'))
sgr.data.1.S$Treatment <- factor(sgr.data.1.S$Treatment, c("II", "SI", "SS", "IS"))
sgr.data.1.S$Tree <- factor(sgr.data.1.S$Tree)
sgr.data.1.S$Position <- factor(sgr.data.1.S$Position)
sgr.data.1.S$Species <- factor(sgr.data.1.S$Species)
sgr.data.1.S$Date <- ordered(sgr.data.1.S$Date, c("03/2023","04/2023", "05/2023", "06/2023"))

# Exclude columns not used
sgr.data.1.S <- sgr.data.1.S %>%
  dplyr::select(-c("Length", "Cause", "Condition", "EV"))

# Add column with observer
constant_value <- "A"
sgr.data.1.S["Observer"] <- constant_value

# Visualize time effect
sgr.data.summary.S <- data_summary(sgr.data.1.S, varname = "SGR", groupnames = c("Species", "Treatment", "Date"))

ggplot(sgr.data.summary.S, aes(x = Date, fill = Treatment, y = SGR))+
  geom_bar(stat = "identity", position = position_dodge(), na.rm = TRUE)+
  labs(y = expression(paste("SGR (", d^-1,")")))+
  scale_y_continuous(expand=c(0, 0), limits=c(-0.004, 0.024))+
  facet_grid(Species ~ Treatment)+
...
## JUDY HALF
# creating data set with only Judys half plots
half.J <- c("II_43", "SI_37", "SI_35", "SI_50", "II_44", "II_37", "II_31", "SI_39", "SI_44", "II_42", "II_33", "SI_43", "II_49", "II_46", "SI_34", "SI_49", "II_32", "SI_36", "SI_33", "II_50", "SS_47", "SS_41", "SS_37", "SS_39", "IS_31", "SS_36", "SS_45", "SS_40", "IS_41", "IS_45", "SS_48", "IS_48", "SS_33", "IS_42", "IS_38", "SS_44", "IS_43", "SS_50", "SS_43", "IS_35")  # select only half of the plot
sgr.data.J <- sgr.data %>% filter(Structure %in% half.J) 

# cleaning data
sgr.data.1.J <- subset(sgr.data.J, Condition >= 80 | is.na(Condition))
sgr.data.1.J <- subset(sgr.data.1.J, Date != "Average") # Exclude Average-data
sgr.data.1.J <- subset(sgr.data.1.J, !(is.na(SGR))) # Exclude SGR with NA
sgr.data.1.J <- sgr.data.1.J %>% separate(Structure, c('Treatment', 'Tree'))
sgr.data.1.J$Treatment <- factor(sgr.data.1.J$Treatment, c("II", "SI", "SS", "IS"))
sgr.data.1.J$Tree <- factor(sgr.data.1.J$Tree)
sgr.data.1.J$Position <- factor(sgr.data.1.J$Position)
sgr.data.1.J$Species <- factor(sgr.data.1.J$Species)
sgr.data.1.J$Date <- ordered(sgr.data.1.J$Date, c("03/2023","04/2023", "05/2023", "06/2023"))

# Exclude columns not used
sgr.data.1.J <- sgr.data.1.J %>%
  dplyr::select(-c("Length", "Cause", "Condition", "EV"))

# Add column with observer
constant_value <- "B"
sgr.data.1.J["Observer"] <- constant_value

# Visualize time effect
sgr.data.summary.J <- data_summary(sgr.data.1.J, varname = "SGR", groupnames = c("Species", "Treatment", "Date"))

ggplot(sgr.data.summary.J, aes(x = Date, fill = Treatment, y = SGR))+
  geom_bar(stat = "identity", position = position_dodge(), na.rm = TRUE)+
  labs(y = expression(paste("SGR (", d^-1,")")))+
  scale_y_continuous(expand=c(0, 0), limits=c(-0.004, 0.024))+
  facet_grid(Species ~ Treatment)+
...
sgr.data.1.summary.J <- data_summary(sgr.data.1.J, varname = "SGR", groupnames = c("Species", "Treatment", "Date","Position"))

# merge data Sara & Judy
sgr.data.1.SJ <- rbind(sgr.data.1.S, sgr.data.1.J)

# plot 
sgr.data.summary.SJ <- data_summary(sgr.data.1.SJ, varname = "SGR", groupnames = c( "Treatment", "Date", "Observer"))

ggplot(sgr.data.summary.SJ, aes(x = Date, fill = Treatment, y = SGR))+
  geom_bar(stat = "identity", position = position_dodge(), na.rm = TRUE)+
  labs(y = expression(paste("SGR (", d^-1,")")), x = "Date")+ facet_grid(rows=vars(Observer)) +
  scale_y_continuous(expand=c(0, 0), limits=c(-0.004, 0.024))+
  geom_errorbar(aes(ymin=SGR-(1*se), ymax=SGR+(1*se)), width=.2,
                position=position_dodge(.9))+
  scale_fill_manual(values=c("#A9A9A9", "#7A7A7A", "#5A5A5A", "#3A3A3A"))+
